...
// БД — сохраняется в 3 места одновременно:
// 1. /data/knb_db.json     — Railway Volume (постоянный, добавь в Railway: Volumes → /data)
// 2. ./knb_db.json         — рядом с server.js (для локальной разработки)
// 3. /tmp/knb_db.json      — временный (запасной)
const DATA_DIR  = process.env.DATA_DIR || '/data';
const DB_PATHS  = [
  path.join(DATA_DIR, 'knb_db.json'),      // Railway Volume — ГЛАВНЫЙ
  path.join(__dirname, 'knb_db.json'),      // рядом с server.js
  '/tmp/knb_db.json',                       // /tmp запасной
];
...
async function saveDB() {
  ...
  _saveTimer = setTimeout(async () => {
    // Сохранить во все 3 места — если одно недоступно, данные не потеряются
    for (const p of DB_PATHS) {
      try {
        await fs.ensureDir(path.dirname(p)).catch(()=>{});
        await fs.writeJson(p, DB);
      } catch(e) { /* путь недоступен — пропускаем */ }
    }
  }, 500);
}

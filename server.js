...
const DB_PATH     = path.join('/tmp', 'knb_db.json');

const app    = express();
const server = http.createServer(app);

// WebSocket через ручной upgrade — работает на Railway
...
function saveDB() {
  ...
  saveTimer = setTimeout(() => fs.writeJson(DB_PATH, DB).catch(e=>console.error('[DB] save:',e.message)), 800);
}
...
// Уведомление + WS в реальном времени
...
app.post('/api/posts/:id/comment', auth, async (req,res) => {
  ...
  post.comments.push({uid:req.user.id,text:req.body.text.trim(),ts:new Date().toISOString()});
...
app.post('/api/friendships', auth, async (req,res) => {
  const {to}=req.body;
  if (!to||to===req.user.id) return res.status(400).json({error:'Неверный запрос'});
...
app.post('/api/messages', auth, async (req,res) => {
  ...
  if (chatId.startsWith('cc_')&&!canPost(chatId.replace('cc_',''),req.user.id)) return res.status(403).json({error:'Нет прав писать в этот чат'});
...
app.post('/api/admin/communities/:id/verify', auth, ownerOnly, async (req,res) => {
  const c=fc(req.params.id); if (!c) return res.status(404).json({error:'Not found'});
  c.verified=!c.verified; saveDB(); res.json({ok:true});
});
...
app.get('/api/health', (_,res) => res.json({ok:true,ts:new Date().toISOString(),clients:clients.size}));
...
async function main() {
  ...
  server.listen(PORT, '0.0.0.0', () => {
    console.log(`[KNB] Running on port ${PORT}`);
  });
}
